...
assert.match(service, /installMinecraftSafetySupervisor/);
assert.match(service, /configureSafeMinecraftMovements/);
assert.match(service, /nextBot\.collectBlock\.movements\s*=\s*configureSafeMinecraftMovements\([\s\S]*?lockCritical:\s*true/, 'collectblock must use locked fluid and movement safety');
assert.match(service, /nextBot\.once\('end', \(reason\) => \{\s*if \(bot !== nextBot\) return;/, 'an old dead connection must not clear the replacement bot state');
