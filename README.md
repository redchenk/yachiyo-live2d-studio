...
Autonomous play now runs as a dedicated gameplay loop instead of waiting for the next live-chat turn. It keeps a persistent survival objective, waits for each queued action to succeed or fail, feeds the outcome into the next LLM plan, and yields while the live-response LLM is busy. The planner can explore, eat, equip gear, sleep and smelt in addition to the original actions, and a deterministic survival fallback keeps gathering wood and building basic tools when the planner API is temporarily unavailable. Meaningful milestones may be spoken through the normal Japanese TTS plus synchronized Chinese-caption path; routine actions remain silent. A host can change the persistent objective from the MC page or a manual prompt, while untrusted viewer text cannot rewrite it.
